...
namespace RPGM.Gameplay
{
    ...
    : MonoBehaviour, ISerializationCallbackReceiver
    {
        [HideInInspector]
        [SerializeField]
        public List<ConversationPiece> items = new List<ConversationPiece>();

        Dictionary<string, ConversationPiece>
            index = new Dictionary<string, ConversationPiece>();
        ...
        public void Add(ConversationPiece conversationPiece)
        {
            ...
            index[conversationPiece.id] = conversationPiece;
        }

        public void Set(
            ConversationPiece originalConversationPiece,
            ConversationPiece newConversationPiece
        )
        {
        ...
        {
            index.Clear();
            foreach (var i in items) index[i.id] = i;
        }
    }
}
