...
namespace RPGM.Gameplay
{
    ...
    public class Quest : MonoBehaviour
    {
        ...
        public ConversationScript
                ...
                questInProgressConversation,
                questCompletedConversation;
            ...
            if (introCutscenePrefab != null)
            {
                ...
                if (cs.audioClip != null)
                {
                    cs.OnFinish += (i) =>
                        model
                            .musicController
                            .CrossFade(model.musicController.audioClip);
                }
            }
            if (enableOnQuestStart != null)
                foreach (var i in enableOnQuestStart)
        ...
        public bool IsQuestComplete()
        {
            ...
            {
                if (
                    inv.Contains(i.item.name) &&
                    model.GetInventoryCount(i.item.name) >= i.count
                ) continue;
                return false;
            }
            return true;
        ...
        public async void RewardItemsToPlayer()
        {
            foreach (var i in rewardItems)
            {
                // If quest title is "Lost Chickens"
                // Do some special stuff:
                if (title == "Lost Chickens")
                {
